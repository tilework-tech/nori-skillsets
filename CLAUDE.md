...
- **Almost always** means modifying files in `src/cli/agents/claude/` (the Nori Plugin package)
- **Rarely** means modifying `~/.claude/` (the installed user configuration)

**Default assumption:** Unless explicitly stated otherwise, "modify Claude configuration", "update skills", "change hooks", etc. refers to modifying the Plugin package source files at `src/cli/agents/claude/`, NOT the installed configuration at `~/.claude/`.

**Examples:**

- "Update the writing-skills skill" → Modify `src/cli/agents/claude/profiles/config/_mixins/_base/skills/writing-skills/SKILL.md`
- "Change the status line" → Modify `src/cli/agents/claude/statusline/`
- "Add a new slash command" → Modify `src/cli/agents/claude/profiles/config/_mixins/_base/slashcommands/`
...
Skills in src/cli/agents/claude/profiles/ are self-explanatory. Do not create docs.md files in skill directories unless the skill is particularly complex and requires additional context files.
