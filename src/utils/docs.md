...
**`path.ts`** resolves installation directories through a priority chain: CLI flag > config value > home directory. `resolveInstallDir()` returns a `ResolvedInstallDir` object with `{ path: string; isOverride: boolean }`. The `isOverride` flag is `true` when the path came from the CLI `--install-dir` flag, which is the single source of truth for whether manifest operations should be skipped. Callers that need to skip manifest operations (e.g., `switchSkillset`, `registryInstall`, `install`) destructure `isOverride` from the result and thread it through as `skipManifest` at the Agent interface boundary. Callers that only need the path (e.g., `clear`, `installLocation`, `registryDownload`) destructure just `{ path }`. The underlying `normalizeInstallDir()` handles tilde expansion, relative-to-absolute conversion, and strips trailing agent config directory segments (e.g., `.claude`) when `agentDirNames` is provided.
