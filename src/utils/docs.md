...
The path.ts module exports `normalizeInstallDir` and `getInstallDirs` for installation directory management.
...
**getInstallDirs Architecture:** This function accepts { currentDir?: string | null } and returns an array of paths to directories with Nori managed installations, ordered from closest to furthest (empty array if none found). It searches the current directory first (defaulting to `process.cwd()` if not provided), then walks up ancestor directories starting from the immediate parent. Detection uses a single criterion: `.claude/CLAUDE.md` must exist and contain the "NORI-AI MANAGED BLOCK" marker. This function is used by init, install-location, and hooks to locate managed installations.
...
**getInstallDirs detection criterion:** The function only checks for managed CLAUDE.md blocks (`.claude/CLAUDE.md` containing "NORI-AI MANAGED BLOCK"). It does not check for `.nori-config.json` files. This is a simplification from earlier versions which also checked for config file presence and classified installations by type ("source", "managed", "both"). The config is now always at `~/.nori-config.json` so there is no need for directory-walking config discovery.
