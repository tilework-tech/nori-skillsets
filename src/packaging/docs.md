...
- `createArchive` writes a temp `.tgz` next to (not inside) the source directory, always deletes it in a `finally`, follows symlinked files/directories while building the pack list so linked skillsets upload real content, and filters entries through the shared upload exclusion predicate. A local Git-backed skillset can therefore be uploaded as an ordinary Registrar tarball: `.git` directories and worktree pointer files are omitted, but authored files such as `.gitignore` are included. That upload does not make the local working tree Registrar-managed; future changes to it still use Git lifecycle operations.
