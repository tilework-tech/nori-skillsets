...
- `_base`: Core infrastructure (using-skills, creating-skills, web-search-researcher, nori-debug/info/nori-switch-profile commands). The creating-skills skill provides an interactive workflow for users to create custom skills, guiding them through requirements gathering, profile selection, directory creation, SKILL.md writing, optional script bundling, verification, and profile switching.
