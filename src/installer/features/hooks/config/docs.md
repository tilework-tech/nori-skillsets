...
**nested-install-warning.ts**: SessionStart hook that warns when multiple Nori installations exist in the directory tree (current directory + ancestors). Loads disk config to get the installation directory (defaulting to cwd if not set), normalizes it using normalizeInstallDir(), then calls findAllInstallations() from @/plugin/src/utils/path.ts to detect ALL directories with Nori installations (including the current directory). Only warns when `allInstallations.length >= 2` - this prevents false warnings when there is a single installation in a parent directory but no installation in the current directory. When multiple installations are detected, builds a systemMessage warning about Claude Code's recursive CLAUDE.md loading behavior and lists ALL installation paths (not just ancestors) with uninstall commands for each. Changed warning header from "Existing Nori installations found at:" to "Nori installations found at:" to reflect that all installations are listed. The hook uses logToClaudeSession() helper to output JSON with systemMessage field for Claude Code to display. All errors are caught and logged silently with exit code 0 to prevent disrupting session startup.
...
**Nested installation warning bug fix**: Prior to v15.3.0, nested-install-warning.ts used findAncestorInstallations() which only checked parent directories, and warned whenever `ancestorInstallations.length > 0`. This caused false warnings when a user had a single Nori installation in a parent directory (e.g., `/home/user/.claude`) but was working in a subdirectory with no installation (e.g., `/home/user/code/project`). The hook incorrectly treated this as a "nested" scenario when in reality only ONE installation existed. The fix introduced findAllInstallations() which checks the entire directory tree INCLUDING the current directory, and changed the warning logic to only trigger when `allInstallations.length >= 2`. This ensures warnings only appear when there are genuinely multiple installations that would cause Claude Code to load duplicate CLAUDE.md files. The function findAncestorInstallations() remains unchanged and continues to be used correctly by install.ts and autoupdate.ts for their specific use cases (one-time installation warning and config file discovery respectively).
