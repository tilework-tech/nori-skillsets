...
Path: @/plugin/src/cli/features/profiles/config/_mixins/_docs/subagents
...
These subagent .md files are copied to ~/.claude/agents/ during profile installation by @/plugin/src/cli/features/subagents/. The main agent invokes these subagents using the Task tool with a subagent_type parameter. The /nori-init-docs slash command (defined in @/plugin/src/cli/features/profiles/config/_mixins/_docs/slashcommands/) invokes nori-initial-documenter, while /update-noridocs invokes nori-change-documenter. Both subagents integrate with the nori-sync-docs skill (from @/plugin/src/cli/features/skills/config/paid/meta/nori-sync-docs/) to push documentation to remote servers. These subagents are referenced in profile CLAUDE.md files (like @/plugin/src/cli/features/profiles/config/amol/CLAUDE.md) as part of the documentation workflow.
