...
- Holds the global install lock across the full interactive or non-interactive switch transaction, including every agent's switch/reinstall work and the final config commit; nested `installMain` calls inherit the transaction reentrantly
