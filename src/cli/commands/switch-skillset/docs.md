...
- Optionally triggers `registryDownloadMain` from `@/cli/commands/registry-download/registryDownload.js` to re-download the target skillset before switching (controlled by `config.redownloadOnSwitch`, and gated to **registry-backed** skillsets via `skillsetHasRegistrySource` from @/src/packaging/provenance.ts)
...
| `onRedownload` | Re-downloads the skillset from the registry (omitted when `redownloadOnSwitch` is disabled; a no-op for skillsets with no registry source — the locally-created `personal/` bucket) |
...
- **Re-download only applies to registry-backed skillsets.** The `onRedownload` callback resolves the target dir and checks `skillsetHasRegistrySource` (from @/src/packaging/provenance.ts) before calling `registryDownloadMain`; a locally-created `personal/` skillset has no registry source and is skipped, because handing `personal/<name>` to the registry parser would misroute it to a nonexistent `personal` org registry.
- The bare-name deprecation warning is emitted via `resolveUserSkillsetRef({ name, warn: !nonInteractive })`, so `switch --non-interactive` (automated fleet provisioning) suppresses the nudge while interactive switches still surface it once per process.
