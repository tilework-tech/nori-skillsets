...
After the installation check, auth is always loaded from the single centralized config via `loadConfig()` (zero-arg). The skillsets directory is always `getNoriProfilesDir()` (zero-arg, returns `~/.nori/profiles/`).
