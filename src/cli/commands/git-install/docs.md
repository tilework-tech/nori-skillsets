...
- Validation parses NUL-delimited `git ls-files --stage -z` records before reading metadata, so unusual tracked pathnames cannot alter the safety checks. It rejects symbolic links, submodules, and any compatibility-normalized, case-insensitive spelling of a root `.nori-version`. It also requires exactly one compatibility-equivalent root manifest entry: the exact lowercase `nori.json` tracked as a regular file. This prevents filesystem aliases, colliding manifest entries, or manifest symlinks from becoming the authority. Clone and validation errors remove the reserved destination, leaving no partial local skillset.
...
- Unpinned installs intentionally accept a shallow source, remain attached to `skillsets/<slug>`, and activate the observed tip. Only historical pinning requires enough history to prove ancestry.
