...
- The Git process boundary closes child stdin for every command. An `EPIPE` caused by Git exiting before consuming that input is treated as a normal early close; other stdin failures remain part of the shared Git error result.
