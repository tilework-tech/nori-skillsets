...
- Acquisition and validation failures clean up the checkout. Activation failures instead retain the validated checkout and return a recovery command that preserves an explicit install-directory override and the effective single-agent scope. Dynamic recovery arguments are POSIX-shell-quoted so copy/paste replays paths and identities literally without shell expansion. Activation otherwise follows the existing installer contract: a successful earlier agent may already have written output and persisted the active skillset before a later agent fails.
