...
- Uses `@clack/prompts` for console output in `watchStopMain` (`clack.log.success`, `clack.log.warn`, `clack.log.info`) and delegates interactive flow to `watchFlow` from @/src/cli/prompts/flows/watch.ts in `watchMain`
...
- Interactive mode delegates to `watchFlow` from @/src/cli/prompts/flows/watch.ts, which handles org selection via `@clack/prompts` `select()`. Two callbacks are provided: `onPrepare` (stops existing daemon, loads config, returns private orgs) and `onStartDaemon` (saves config if destination changed, spawns the daemon process).
- `watchStopMain` uses `@clack/prompts` `log.success()`, `log.warn()`, `log.info()` for console output.
