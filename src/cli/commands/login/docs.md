...
- The `auth.organizations` field is set to `[orgId]` on API-token login (where `orgId` is parsed from the token) even though the server's `req.user` for API-key requests synthesizes `organizations: [{ id: apiKey.orgId, roles: ["user"] }]`. This makes the client-side unified registry-auth check (used by search, upload, download, skill-download, skill-upload, and subagent-download) behave consistently with Firebase sessions and broker-managed session-machine configs. The check accepts `auth.refreshToken`, `auth.apiToken`, or an unexpired `auth.idToken` alongside `auth.organizations`.
