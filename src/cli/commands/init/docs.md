...
**Marker file ownership:** `initMain` is the sole writer of `.nori-managed` marker files (via `markInstall` from @/src/cli/features/agentOperations.ts). It accepts an optional `skillset` parameter that callers (e.g., `noninteractive()` in @/src/cli/commands/install/install.ts) use to thread the intended skillset name through. The non-interactive path resolves the marker's skillset name via the chain `capturedSkillsetName ?? skillset ?? existingConfig?.activeSkillset ?? null`. This ensures that during `nori switch`, where `capturedSkillsetName` is null but the caller knows the target skillset, the marker still receives the correct value.
