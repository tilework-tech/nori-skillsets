...
- After installation, iterates all agents via `AgentRegistry.getInstance().getAll()` and calls `markInstall()` on each, marking the directory as having agents installed
...
- **Existing installation skip**: Both the interactive and non-interactive paths check if any registered agent is installed via `AgentRegistry.getInstance().getAll().some(agent => agent.isInstalledAtDir(...))` to skip existing-config detection. This prevents re-capturing configuration on subsequent inits and is agent-agnostic -- adding a new agent to the registry automatically participates in this check.
