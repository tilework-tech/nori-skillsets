...
This command is the entry point for new installations and is also called implicitly by the `registry-download` command when no config exists. It interacts with `@/cli/config.js` for loading/saving the global config, `@/cli/features/agentRegistry.js` for agent configurations, `@/cli/features/shared/agentHandlers.js` for shared handler functions (isInstalledAtDir, detectExistingConfig, captureExistingConfig, markInstall), `@/cli/features/paths.js` for shared Nori directory locations, and `@/cli/prompts/flows/init.js` for the interactive setup wizard.
