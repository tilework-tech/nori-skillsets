...
- Resolves the default agent using `getDefaultAgent()` from @/src/cli/config.js and `AgentRegistry.getInstance().get({ name })`
...
- **Default agent resolution**: At the start of `initMain`, resolves the default agent by loading existing config (if any), calling `getDefaultAgent({ config })`, and looking up the agent via `AgentRegistry.getInstance().get({ name })`. All agent-specific operations use this resolved agent.
...
- **Agent-agnostic init**: Init is now agent-agnostic and delegates all agent-specific operations through the default agent's interface. The agent name is resolved from `config.defaultAgent` via `getDefaultAgent()`, making init work with any agent that implements the Agent interface. Future agents can participate in init by implementing the optional `detectExistingConfig` and `captureExistingConfig` methods.
- **Config object for capture**: When calling `defaultAgent.captureExistingConfig`, init builds a `Config` object with `installDir` and `agents` fields. The `agents` object uses `defaultAgent.name` as the key (e.g., `{ "claude-code": { profile: { baseProfile: profileName } } }`), ensuring the captured profile is activated for the correct agent.
