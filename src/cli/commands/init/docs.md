...
**Multi-agent broadcasting:** When capturing existing config or marking installation complete, the command loops over ALL default agents (via `getDefaultAgents()`, which automatically incorporates the global `--agent` flag override set by the CLI `preAction` hook -- see @/src/cli/docs.md) and calls the operation on each agent. Both `captureExistingConfig` and `markInstall` are broadcast to all agents in the default set, ensuring consistency across multi-agent setups.

**Marker file ownership:** `.nori-managed` marker files are written only by init-time code (via `markInstall` from @/src/cli/features/agentOperations.ts): the interactive path's `onInit` callback and the shared `ensureNoriInitialized` core in @/src/cli/features/install/initialize.ts. `initMain` accepts an optional `skillset` parameter that callers use to thread the intended skillset name through to `ensureNoriInitialized`, which resolves the marker's skillset name via the chain `capturedSkillsetName ?? skillset ?? existingConfig?.activeSkillset ?? null`. This ensures that during `nori switch`, where `capturedSkillsetName` is null but the caller knows the target skillset, the marker still receives the correct value.
