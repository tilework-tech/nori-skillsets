...
describe("init command", () => {
  ...
  describe("initMain", () => {
    it("initializes storage without creating config or claiming agent state", async () => {
      const unmanagedInstructions = path.join(TEST_CLAUDE_DIR, "CLAUDE.md");
      ...
      await expect(
        fsPromises.access(path.join(TEST_NORI_DIR, "profiles")),
      ).resolves.toBeUndefined();
      await expect(fsPromises.access(getConfigPath())).rejects.toThrow();
      await expect(
        fsPromises.readFile(unmanagedInstructions, "utf8"),
      ).resolves.toBe("# Existing unmanaged instructions\n");
      await expect(
        fsPromises.access(path.join(TEST_CLAUDE_DIR, ".nori-managed")),
      ).rejects.toThrow();
    });
