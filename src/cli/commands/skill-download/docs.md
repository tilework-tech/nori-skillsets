...
The `--skillset` flag lets the user target a specific skillset for manifest updates; otherwise it defaults to the active skillset from config. The target skillset's on-disk directory is located via `resolveSkillsetDir` from @/src/norijson/skillset.ts (so a bare name resolves across the `personal/`/`public/` storage buckets), and the command tracks the resolved directory (`targetSkillsetDir`) separately from the bare display name (`targetSkillset`) — persistence writes go to the resolved directory while messages show the bare name. Like registry-download, updates use the atomic swap with backup/restore on failure from @/src/packaging/atomicReplace.ts, and failed fresh installs clean up their partial directory. The `--registry` flag and namespace prefix (`org/`) are mutually exclusive since the namespace implicitly determines the registry URL. The `nonInteractive` and `silent` params are threaded from the CLI registration layer through `skillDownloadMain` to `skillDownloadFlow`, where they control whether the "Re-download from registry?" confirm prompt is skipped when the skill is already at the current version. The coercion `nonInteractive ?? silent ?? false` is applied so `--silent` implies non-interactive behavior.
