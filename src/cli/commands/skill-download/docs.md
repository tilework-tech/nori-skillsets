...
The complete download, profile-manifest update, and live multi-agent copy hold the reentrant global mutation lock, preventing interleaving with activation, cleanup, config mutation, or another live download. This serialization does not roll back earlier writes if a later agent copy fails.
