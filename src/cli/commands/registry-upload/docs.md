...
- Tarball creation is delegated to `createArchive` from @/src/packaging/archive.ts, which resolves a top-level linked skillset, rejects it when the resolved root is at or beneath Git metadata, filters entries through the shared `shouldExcludeFromUpload` predicate (including literal `.git`, `.nori-version`, editor/OS junk, and dependency/build output), and rejects upload-eligible symbolic links anywhere below an accepted root. It packs only its explicit regular-file list with symlink following and directory recursion disabled, collecting the result in memory instead of using a predictable sibling temp file. A skillset registered through `sks link` remains uploadable when it targets an ordinary package directory, but roots linked into `.git` and linked skills or subagents inside the package must not be uploaded. The same primitive is used by `@/src/cli/commands/skill-upload/skillUpload.ts`, so both upload paths share this boundary and exclusion list. See @/src/packaging/docs.md.
- Directory discovery elsewhere in the upload flow still uses `isDirentDirectory()` from `@/src/utils/dirent.ts` where linked directories must be recognized for resolution and migration logic. Recognition does not make an interior link archive-eligible: `createArchive` is the final package boundary and rejects it before any Registrar upload.
