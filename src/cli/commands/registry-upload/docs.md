...
- Tarball creation is delegated to `createArchive` from @/src/packaging/archive.ts, which resolves a top-level linked skillset, filters entries through the shared `shouldExcludeFromUpload` predicate (including literal `.git`, `.nori-version`, editor/OS junk, and dependency/build output), and rejects upload-eligible symbolic links anywhere below the resolved root. It packs only its explicit regular-file list with symlink following and directory recursion disabled, collecting the result in memory instead of using a predictable sibling temp file. A skillset registered through `sks link` remains uploadable, but linked skills or subagents inside it must be materialized before whole-skillset upload. The same primitive is used by `@/src/cli/commands/skill-upload/skillUpload.ts`, so both upload paths share this boundary and exclusion list. See @/src/packaging/docs.md.
