...
- `createProfileTarball` filters tarball entries through the shared `shouldExcludeFromUpload` predicate from `@/src/utils/uploadFileFilter.ts`, which skips local download metadata (`.nori-version`), editor swap/backup files, OS junk, and dependency/build "bloat" directories (`node_modules/`, `.venv/`, and Rust `target/` output) so they cannot pollute the registry's content hash. It first calls `collectCargoManifestDirs` over the recursively-listed files to find directories holding a `Cargo.toml`, then passes that set as `cargoManifestDirs` so a `target/` directory is only dropped when it is adjacent to a Cargo manifest. The same helpers are reused by `@/src/cli/commands/skill-upload/skillUpload.ts` so both upload paths share one exclusion list. The `tar.create()` call uses `follow: true` so symlinked files inside the skillset directory are packed as real file content rather than as symlinks.
