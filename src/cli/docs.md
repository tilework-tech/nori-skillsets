...
**Startup import graph:** Command registration in `@/cli/commands/noriSkillsetsCommands` is eager (so commander sees every command for `--help`), but each command's `*Main` handler module is loaded lazily via `await import()` inside the `.action()` callback. This means `nori-skillsets --version` and `nori-skillsets list-active` skip the handler graph for heavy commands like `login`, `registry-search`, `registry-download`, `registry-install`, and `registry-upload`. The eager side of startup still includes `AgentRegistry` (imported from the features barrel), which transitively pulls in `configLoader` and `firebase/auth`, so cold start is not free.
