...
**CLI Architecture:** The package provides a CLI binary (defined in @/package.json bin):
...
```
src/cli/
  nori-skillsets.ts      # CLI entry point with all commands
...
| `search` | commands/registry-search/registrySearch.ts | Search for skillsets and skills in the Nori registrar |
| `download` | commands/registry-download/registryDownload.ts | Download and install a skillset from the Nori registrar |
| `switch-skillset` | commands/switch-profile/profiles.ts | Switch to a different profile/skillset |
| `download-skill` | commands/skill-download/skillDownload.ts | Download a skill from the Nori registrar |
...
This ensures that when running `cd /foo/bar && npx nori-skillsets init`, all files are created in `/foo/bar/` rather than the user's home directory. Note: The `.nori-installed-version` file has been deprecated - version is now stored in the `version` field of `.nori-config.json`.
...
3. If config exists but has no `version` field, attempts fallback to deprecated `.nori-installed-version` file. If that file exists and contains valid semver, uses it as the version for migration. If no fallback is available, throws an error requiring manual reinstall
