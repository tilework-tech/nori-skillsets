...
CLI for Nori Profiles that prompts for configuration, installs features into Claude Code, manages credentials, and tracks installation analytics via Google Analytics. All features are available regardless of authentication status. The CLI uses Commander.js for command routing, argument parsing, and help generation.
...
- `username` and `organizationUrl` - required for authenticated installs
...
**Authentication Status:** Authentication (`config.auth`) exists for registry access and API connectivity but does not gate feature availability. All features are installed for all users regardless of authentication status. The `config.auth != null` check is used only for registry operations and analytics tracking (`"authenticated" | "unauthenticated"`).
