...
/**
 * Migration 19.0.0: Consolidate flat auth fields into nested auth object
 *
...
const migration_19_0_0: Migration = {
  version: "19.0.0",
  name: "consolidate-auth-structure",
  migrate: async (args: {
    config: Record<string, unknown>;
    installDir: string;
  }): Promise<Record<string, unknown>> => {
    ...
    // If config already has nested auth, preserve it (idempotent)
    if (result.auth != null) {
