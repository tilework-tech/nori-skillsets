...
The loader implements `uninstall()` to remove `hooks`, `includeCoAuthoredBy`, and `attribution` keys from `~/.claude/settings.json` via `removeSettingsKeys()` from `@/src/cli/features/claude-code/paths.ts`. This is called by `removeSkillset()` in @/src/cli/features/agentOperations.ts during `sks clear` or `sks clear-current`.
