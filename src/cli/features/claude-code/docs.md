...
Claude Code agent implementation that satisfies the Agent interface from @/src/cli/features/agentRegistry.ts. Contains feature loaders and configurations for installing Nori components into Anthropic's Claude Code CLI tool. Uses a directory-based skillset system where each skillset is self-contained with CLAUDE.md, skills, subagents, and slash commands. Contains loaders for: config, skillsets, hooks, statusline, and announcements. Claude-specific path helpers are encapsulated in paths.ts within this directory.
...
Each loader implements run(config) to install. The skillsets loader (@/src/cli/features/claude-code/skillsets/loader.ts) orchestrates skillset-dependent features through a ProfileLoaderRegistry that manages sub-loaders for claudemd, skills, slashcommands, and subagents within each skillset.
...
- `captureExistingConfigAsSkillset()`: Copies existing skills, agents, commands, and CLAUDE.md content (excluding the managed block if present) into `~/.nori/profiles/{skillsetName}/`, creating a self-contained skillset directory
...
**Self-contained skillsets**: Each skillset is a complete, standalone directory containing all content directly (CLAUDE.md, skills/, subagents/, slashcommands/). No mixin composition or inheritance is used. The package does not ship any built-in skillsets -- skillsets are obtained from the registry or created by users.
...
The LoaderRegistry provides getAll() for install order. The skillsets loader must run first because other loaders read from the skillset directories it creates.
...
- Namespaced skillsets: `~/.nori/profiles/{org}/{profile-name}/` - for organization-specific registry packages

The `listProfiles()` function in @/src/cli/features/managedFolder.ts discovers both layouts by first checking if a directory contains `nori.json` (flat profile), and if not, checking for subdirectories that contain `nori.json` (org directory with nested profiles). Namespaced skillsets are returned in `org/profile-name` format.

**Profile structure**: Each skillset directory contains CLAUDE.md, skills/, subagents/, slashcommands/, and nori.json (unified manifest). All content is self-contained - no mixin composition or inheritance.
...
**Switch skillset**: The switch-skillset command updates nori-config.json and re-runs installation to apply the new skillset. Most changes require Claude Code restart except CLAUDE.md which applies to new conversations immediately.
