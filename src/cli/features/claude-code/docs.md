...
- `switchSkillset({ installDir, skillsetName })`: Validates skillset exists (handles both flat and namespaced paths via `path.join`), calls `updateConfig({ activeSkillset: skillsetName })` to persist the change, and logs success message. The `installDir` argument is accepted (required by the `Agent` interface) but is not persisted -- `updateConfig()` preserves the existing `installDir` from disk. Imports `MANIFEST_FILE` from @/src/cli/features/managedFolder.ts to identify valid skillsets.
