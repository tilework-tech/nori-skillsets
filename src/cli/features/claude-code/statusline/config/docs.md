...
**Config Reading:** Once `.nori-config.json` is located, the script reads two pieces of data from it: the profile name (`activeSkillset` with fallback to legacy `agents.claude-code.profile.baseProfile` and `profile.baseProfile`) and the installed version (`.version` field). Both are used downstream -- profile for the metrics line, version for the branding line and update comparison.
