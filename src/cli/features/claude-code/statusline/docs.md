...
**Profile Display:** Profile name is conditionally displayed only when .nori-config.json contains an `activeSkillset` value (with fallback to legacy `agents.claude-code.profile.baseProfile` path). When not set, the profile section is omitted from the metrics line.
