...
Feature loader for installing the status line script into Claude Code, enabling real-time display of git branch, active skillset name, token usage, conversation costs, installed version, and a promotional tip for the Nori CLI.
...
**Script Execution:** The status line script is invoked by Claude Code during conversation updates and receives conversation data via stdin. At runtime, the script walks upward from the CWD to locate `.nori-config.json`, which it reads for both skillset information and version. The script displays three lines: metrics (git branch, optional skillset name, cost, tokens, context, lines), branding ("Augmented with Nori" with version if available from config), and a status tip (update notification or promotional tip).
...
**Skillset Display:** Skillset name is conditionally displayed only when .nori-config.json contains an `activeSkillset` value. When not set, the skillset section is omitted from the metrics line.
