...
export const clineAgentConfig: AgentConfig = {
  name: "cline",
  displayName: "Cline",
  description: "Instructions, skills, subagents, commands",

  getAgentDir: ({ installDir }) => path.join(installDir, ".cline"),
  getSkillsDir: ({ installDir }) => path.join(installDir, ".cline", "skills"),
  getSubagentsDir: ({ installDir }) =>
    path.join(installDir, ".cline", "agents"),
  getSlashcommandsDir: ({ installDir }) =>
    path.join(installDir, ".cline", "commands"),
  getInstructionsFilePath: ({ installDir }) =>
    path.join(installDir, ".cline", "rules", "AGENTS.md"),
