...
- `_base`: Core infrastructure (using-skills, creating-skills, web-search-researcher, nori-debug/info/nori-switch-profile/nori-toggle-autoupdate/nori-modify-registry-auth/nori-modify-watchtower-auth commands). The creating-skills skill provides an interactive workflow for users to create custom skills, guiding them through requirements gathering, profile selection, directory creation, SKILL.md writing, optional script bundling, verification, and profile switching. The nori-toggle-autoupdate command toggles the autoupdate setting in `.nori-config.json` between 'enabled' and 'disabled', allowing users to control whether Nori Profiles automatically updates on session start. The nori-modify-registry-auth command provides an interactive wizard for managing private registry authentication credentials in `.nori-config.json` - users can add new registry auth entries (with URL validation requiring HTTPS) or remove existing ones. The nori-modify-watchtower-auth command provides an interactive wizard for managing Nori Watchtower authentication credentials (username, password, organizationUrl) stored at the root level of `.nori-config.json` - users can add/update credentials or remove them. Unlike registry auth which requires HTTPS, Watchtower auth allows both HTTP and HTTPS URLs (to support local development).
