...
- The instructions loader factory (`createInstructionsLoader`) is parameterized differently depending on whether the instructions file lives at the root of the agent dir (`managedFiles: ["AGENTS.md"]`) vs. in a subdirectory (`managedDirs: ["rules"]` for Cursor). Note that Cline also places its instructions at `rules/AGENTS.md` but uses `managedFiles: ["AGENTS.md"]` rather than `managedDirs: ["rules"]` -- its instructions loader tracks the individual file rather than the directory.
