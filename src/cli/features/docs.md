...
- `captureExistingConfig`: Captures existing config as a named skillset (writing `AGENTS.md` as the config file), deletes the original instructions file, then runs the instructions loader to restore a managed block.
