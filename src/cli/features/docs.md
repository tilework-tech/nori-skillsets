...
Agent abstraction layer that defines the Agent interface and registry for AI agent implementations. Currently supports only Claude Code. Contains shared types (`Loader`, `ValidationResult`, `LoaderRegistry`) that all agents implement, plus the shared config loader that all agents must include. Also contains shared test utilities (@/src/cli/features/test-utils/) used across agent and command tests.

### How it fits into the larger codebase

The features directory sits between the CLI commands (@/src/cli/commands/) and the agent-specific implementation (@/src/cli/features/claude-code/). CLI commands use the AgentRegistry to look up the agent implementation, then delegate to the agent's loaders and profile methods.

```
CLI Commands (install, switch-profile)
    |
    +-- AgentRegistry.getInstance().get({ name: "claude-code" })
    |
    +-- Agent interface
        |
...
The AgentRegistry only supports "claude-code" as a valid agent. Per-agent profile configuration is stored in the Config `agents` field for backwards compatibility.
...
| Type | Purpose |
|------|---------|
| `AgentName` | Type alias for canonical agent identifier (`"claude-code"`). Used as the registry key and source of truth for agent identity. |
...
**`AgentName` is the canonical UID for agents.** The `AgentName` type (`"claude-code"`) is the source of truth for valid agent identifiers. Functions that need the agent identifier access `agent.name` rather than receiving a separate string parameter.
...
The AgentRegistry auto-registers the claude-code agent in its constructor.
