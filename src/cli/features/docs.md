...
- `removeSkillset`: Reads per-agent manifest, calls `removeManagedFiles()`, includes legacy manifest cleanup when the agent declares `getLegacyManifestPath` (declared as data on the claude-code table row -- shared code never branches on agent name), iterates over all loaders calling each loader's optional `uninstall()` method for non-settings cleanup, and then restores external settings files from their pre-install backups via `restoreSettingsFile()` from `@/src/cli/features/settingsBackup.ts`. This full-file restore replaces the old approach of per-loader surgical key removal from settings files.
- `detectLocalChanges`: Reads per-agent manifest (falling back to `agent.getLegacyManifestPath?.()` when declared), compares file hashes.
...
**Other shared modules**: `template.ts` (placeholder substitution), `manifest.ts` (file-level change tracking with SHA-256 hashes; also exports `getLegacyManifestPath`, referenced from the claude-code table row), `bundled-skillsets/` (bundled skills installer), `shared/managedDirOps.ts` (`resetManagedDir` helper used by the skills/slashcommands/subagents loaders to clear their destination directory before repopulating it while preserving any top-level dotfile entries).
