...
describe("install noninteractive", () => {
  ...
  it("rejects without exiting when no skillset is available", async () => {
  ...
  it.runIf(process.platform === "linux")(
    "reclaims a lock whose PID belongs to a different process incarnation",
    async () => {
      const lockPath = path.join(getHomeDir(), ".nori-install.lock");
      await fsPromises.mkdir(lockPath);
      await fsPromises.writeFile(
        ...
        JSON.stringify({ processIdentity: "different-boot:different-start" }),
      );

      await expect(
        withInstallLock({ operation: async () => "recovered" }),
      ).resolves.toBe("recovered");
      await expect(fsPromises.access(lockPath)).rejects.toThrow();
    },
  );

  it("reclaims an empty lock directory left by interrupted cleanup", async () => {
    const lockPath = path.join(getHomeDir(), ".nori-install.lock");
    await fsPromises.mkdir(lockPath);
    const expiredAt = new Date("2000-01-01T00:00:00.000Z");
