...
|---|---|---|
| `CLAUDE.md` | `.claude/CLAUDE.md` | `.cursor/AGENTS.md` |
...
**AGENTS.md generation** (`skillsets/agentsmd/loader.ts`): Reads the skillset's `CLAUDE.md` (via `skillset.configFilePath`), strips any existing managed block markers, applies template substitution with the `.cursor` install directory, appends a generated skills list section (by scanning SKILL.md files for front matter metadata), and writes the result into a `# BEGIN NORI-AI MANAGED BLOCK` / `# END NORI-AI MANAGED BLOCK` section in `.cursor/AGENTS.md`. If no config file exists, clears the managed block.

**Path helpers** (`paths.ts`): Provides `getCursorDir`, `getCursorAgentsMdFile`, `getCursorSkillsDir`, `getCursorCommandsDir`, and `getCursorAgentsDir` -- all derived from `{installDir}/.cursor/`.
