...
describe("cursor-agent profiles loader", () => {
  ...
  describe("injectConditionalMixins", () => {
    test("should inject paid mixin for paid user", () => {
      const metadata = {
        name: "test-profile",
    ...
    test("should inject swe-paid mixin for paid user with swe category", () => {
      const metadata = {
        name: "test-profile",
        description: "Test profile",
        mixins: {
    ...
    test("should not inject tier-specific mixins for free user", () => {
      const metadata = {
        name: "test-profile",
        description: "Test profile",
        mixins: {
      ...
      expect(Object.keys(result.mixins).sort()).toEqual(["base", "swe"]);
    });

    test("should not duplicate tier-specific mixin if already present", () => {
      const metadata = {
        name: "test-profile",
        description: "Test profile",
        mixins: {
      ...
      expect(result.mixins).toHaveProperty("paid");
      expect(result.mixins).toHaveProperty("swe-paid");
      // Should only have one swe-paid entry
      expect(
        Object.keys(result.mixins).filter((k) => k === "swe-paid"),
      ).toHaveLength(1);
    });

    test("should not inject tier mixins for base or paid categories", () => {
      const metadata = {
        name: "test-profile",
        description: "Test profile",
        mixins: {
