/**
 * Tests for subagent download flow module
 *
 * These tests verify the subagentDownloadFlow function behavior including:
...
describe("subagentDownloadFlow", () => {
  let mockCallbacks: SubagentDownloadFlowCallbacks;
  let spinnerMock: {
  ...
  beforeEach(() => {
    ...
    mockCallbacks = {
      ...
      onDownload: vi.fn().mockResolvedValue({
        ...
        version: "1.0.0",
        isUpdate: false,
        installedTo: "/home/user/.claude/agents/my-subagent.md",
        subagentDisplayName: "my-subagent",
        profileUpdateMessage:
          'Added "my-subagent" to default skillset nori.json',
        warnings: [],
      }),
    };
  });
  ...
  describe("happy path: new download", () => {
    ...
    it("should show download spinner with subagent name", async () => {
      await subagentDownloadFlow({
        subagentDisplayName: "my-subagent",
        callbacks: mockCallbacks,
      });
    ...
    it("should display next steps note with install location", async () => {
      ...
      expect(clack.note).toHaveBeenCalledWith(
        expect.stringContaining("/home/user/.claude/agents/my-subagent.md"),
        "Next Steps",
      );
    });
    ...
    it("should include statusMessage in result", async () => {
      ...
      expect(result?.statusMessage).toContain("my-subagent");
    });
  ...
  describe("happy path: update existing subagent", () => {
    beforeEach(() => {
    ...
    it("should show update-specific download spinner message", async () => {
      ...
      expect(spinnerMock.start).toHaveBeenCalledWith(
        'Updating "my-subagent" from 1.0.0 to 2.0.0...',
      );
    });
  ...
  describe("already current: user confirms re-download", () => {
    ...
    it("should return download result", async () => {
      ...
      expect(result).toEqual({
        version: "1.0.0",
        isUpdate: true,
        statusMessage: 'Updated "my-subagent" to 1.0.0',
      });
    });
  });
  ...
  describe("search error", () => {
    beforeEach(() => {
      mockCallbacks.onSearch = vi.fn().mockResolvedValue({
        status: "error",
        error: 'Subagent "my-subagent" not found in any registry.',
      });
    });
    ...
    it("should show error message", async () => {
      ...
      expect(clack.log.error).toHaveBeenCalledWith(
        'Subagent "my-subagent" not found in any registry.',
      );
    });
  ...
  describe("download error", () => {
    beforeEach(() => {
      mockCallbacks.onDownload = vi.fn().mockResolvedValue({
        success: false,
        error: 'Failed to download subagent "my-subagent": network timeout',
      });
    });
    ...
    it("should show error message", async () => {
      ...
      expect(clack.log.error).toHaveBeenCalledWith(
        'Failed to download subagent "my-subagent": network timeout',
      );
    });
  ...
  describe("download with warnings", () => {
    beforeEach(() => {
      mockCallbacks.onDownload = vi.fn().mockResolvedValue({
        ...
        subagentDisplayName: "my-subagent",
        warnings: [
          "Warning: Could not persist subagent to profile: permission denied",
