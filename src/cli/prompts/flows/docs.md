...
`uploadFlow` handles subagent conflict resolution after skill conflicts (Step 3b). The `UploadResult` discriminated union includes a `subagentConflicts` variant checked by `hasSubagentConflicts()` — this is the flow-internal representation assembled by the caller from the API client's thrown `SubagentCollisionError`; it is NOT the wire shape. The registrar returns both conflict kinds under a single `conflicts` key on the wire and the API client in `@/src/api/registrar.ts` discriminates by item shape before throwing `SkillCollisionError` vs `SubagentCollisionError` (see `@/src/api/docs.md`). Unchanged subagents are auto-resolved to "link" (mirroring the skill pattern), while modified subagents go through interactive per-item resolution. The subagent conflict resolver adapts `SubagentConflict` objects to the `SkillConflict` shape to reuse `buildResolutionOptions` and `getDefaultAction`. The `onReadLocalSubagentMd` callback enables "View Diff" for subagent conflicts. Linked subagent versions are tracked in `linkedSubagentVersions: Map<string, string>` on `UploadFlowResult`. The upload summary note includes a "Subagents:" section showing extracted/failed subagents.
