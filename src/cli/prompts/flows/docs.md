...
The `link` ("Use Existing") action has two presentations. When `conflict.contentUnchanged === true`, it is labeled with a `Link to existing vX.Y.Z` hint and is the default selection. When content has diverged, the hint reads "Use existing version already on registry. Note that this will discard any local changes." and the flow records the server-supplied `conflict.existingSkillMd` into `linkedSkillsToReplace: Map<string, string>` on the `UploadFlowResult`. The sync layer (in `@/cli/commands/registry-upload`) uses this map to overwrite the local `SKILL.md` with the registry's canonical bytes after upload, so the "discard local changes" promise is realized and the next upload does not re-detect the same conflict. Auto-resolved links never populate `linkedSkillsToReplace` because their content is unchanged by definition. The same pattern applies to subagents via `linkedSubagentsToReplace` (keyed by subagent ID, carrying `conflict.existingSubagentMd`).

`UploadFlowResult` fields related to conflict resolution: `linkedSkillVersions` / `linkedSubagentVersions` (linked ID → remote version, for dependency map updates), `linkedSkillsToReplace` / `linkedSubagentsToReplace` (linked ID → canonical `SKILL.md` / `SUBAGENT.md` content, for on-disk replacement), and `namespacedSkillIds` (skills that were renamed rather than linked). The older `linkedSkillIds` / `skippedSkillIds` fields no longer exist — the "Skipped:" section of the upload summary was removed, and linked skills (including those with discarded local changes) appear under "Linked (existing):" in the summary.

`uploadFlow` handles subagent conflict resolution after skill conflicts (Step 3b). The `UploadResult` discriminated union includes a `subagentConflicts` variant checked by `hasSubagentConflicts()`. Unchanged subagents are auto-resolved to "link" (mirroring the skill pattern), while modified subagents go through interactive per-item resolution. The subagent conflict resolver adapts `SubagentConflict` objects to the `SkillConflict` shape to reuse `buildResolutionOptions` and `getDefaultAction`. The `onReadLocalSubagentMd` callback enables "View Diff" for subagent conflicts. Linked subagent versions are tracked in `linkedSubagentVersions: Map<string, string>` on `UploadFlowResult`. The upload summary note includes a "Subagents:" section showing extracted/failed subagents.
