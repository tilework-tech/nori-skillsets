...
- **uploadFlow:** Multi-step upload flow that handles version determination, upload attempt, and skill conflict resolution. Accepts 2 callbacks: `onDetermineVersion` and `onUpload`. Conflict resolution uses a two-tier strategy:
  ...
  The `link` action is presented as "Use Existing" for both unchanged and changed skills. For unchanged skills, the hint shows the linked version. For changed skills, the hint warns that local changes will be discarded. The flow tracks linked, namespaced, and skipped skill IDs separately and returns them in `UploadFlowResult`. The summary note categorizes skills into Uploaded, Linked (existing), Namespaced, Skipped, Inlined, and Failed sections. Supports a `nonInteractive` flag that blocks upload and displays an error note when unresolvable conflicts exist.

  **Inline skill handling:** The flow accepts both `inlineCandidates` (new skills without nori.json that need a user decision) and `previouslyInlinedSkillIds` (skills already marked inline in the profile's nori.json `skills[]` array). Previously inlined skills are auto-included in the `inlineSkillIds` result without prompting. New candidates are prompted via the same batch/individual resolution pattern used for conflicts. The caller (@/cli/commands/registry-upload/registryUpload.ts) is responsible for parsing nori.json and splitting candidates into these two categories.
