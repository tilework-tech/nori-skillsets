...
`registryDownloadFlow` and `skillDownloadFlow` allow re-downloading packages even when the local version matches the registry version. This supports the case where registry contents change without a version bump (e.g., updated skill dependencies). The confirm prompt defaults to `false` to preserve the previous behavior of skipping the download when already current.
