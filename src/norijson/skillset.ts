...
/**
 * Build the absolute on-disk directory for a namespaced (or bare) skillset name,
 * relative to the profiles root. Splits on `/` so a namespaced identity
 * (`public/foo`, `myorg/foo`) maps to its nested directory. This does NOT check
 * existence — it is the expected location for a name, used as the fallback when
 * resolution finds nothing on disk.
 *
 * @param args - Function arguments
 * @param args.name - The namespaced (or bare) skillset name
 *
 * @returns Absolute path to the skillset directory
 */
export const skillsetPath = (args: { name: string }): string => {
  return path.join(getNoriSkillsetsDir(), ...args.name.split("/"));
};
...
export const skillsetCreateDir = (args: { name: string }): string => {
  const { name } = args;
  if (name.includes("/")) {
    return skillsetPath({ name });
  }
  return path.join(getNoriSkillsetsDir(), PERSONAL_BUCKET, name);
};
