...
- `resolveUserSkillsetRef({ name, activeSkillset?, defaultOrg?, nameWasProvided?, warn? })`: The **local skillset reference** resolution seam. When `nameWasProvided` is true, a bare `name` tries configured `defaultOrg` first (`<defaultOrg>/<name>`), then normal bucket resolution; explicit namespaces such as `public/foo` are never overridden. When `name` is absent, `activeSkillset` is used as an exact current-config fallback and is not reinterpreted through `defaultOrg`. Returns `{ dir, identity }`, emitting a one-time (per process, per name) **deprecation warning** only for provided bare names that reach a bucketed skillset. `warn` defaults to true; callers pass `warn: false` under `--non-interactive` (e.g. `switch --non-interactive` fleet provisioning) so the nudge is not noise. Used wherever a command targets an existing local skillset — `switch`/`edit`/`register` plus skill-download, subagent-download, skill-upload, external, and import-mcp.
