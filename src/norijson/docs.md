...
- `resolveUserSkillsetRef({ name })`: The **user-input** resolution seam. Wraps `resolveSkillsetDir` and returns `{ dir, identity }`, emitting a one-time (per process, per name) **deprecation warning** when a bare name reaches a bucketed skillset (bare references are deprecated in favour of the namespaced identity). Used by the commands where the skillset name is a user-supplied argument (`switch`, `edit`, `register`); internal callers that pass names programmatically use `resolveSkillsetDir` directly so they do not warn.
