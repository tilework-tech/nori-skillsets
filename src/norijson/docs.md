...
- `listSkillsetsWithMetadata()`: Scans `~/.nori/profiles/` for skillset directories, supporting flat and namespaced (org/name) layouts. Uses a private pure `isSkillsetDir` predicate (a `nori.json` is present, or the directory passes `looksLikeSkillset` from `nori.ts`) -- listing is a read and never writes to profiles; it used to call `ensureNoriJson()` per entry, which mutated `~/.nori/profiles` on every list. Uses `isDirentDirectory()` from `@/src/utils/dirent.ts` to follow symlinks when checking directory entries, and reports `isLinked` status via `entry.isSymbolicLink()`. For namespaced entries, a symlink at either the org level or the nested level counts as linked.
