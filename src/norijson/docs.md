...
- `skillsetCreateDir({ name })`: Computes the on-disk directory for a *newly created* local skillset. Bare names land in `personal/`; explicitly namespaced names are written at that namespace unchanged. Used by `new`, `fork`, `external --new`, `captureExistingConfig`, and `link`.
...
- `resolveUserSkillsetRef({ name, warn? })`: The **user-input** resolution seam. Wraps `resolveSkillsetDir` and returns `{ dir, identity }`, emitting a one-time (per process, per name) **deprecation warning** when a bare name reaches a bucketed skillset (bare references are deprecated in favour of the namespaced identity). `warn` defaults to true; callers pass `warn: false` under `--non-interactive` (e.g. `switch --non-interactive` fleet provisioning) so the nudge is not noise. Used wherever the skillset name is a **user-supplied argument** — `switch`/`edit`/`register` plus the explicit `--skillset` branch of skill-download, subagent-download, skill-upload, external, and import-mcp. The silent active-skillset fallback and internal callers that pass names programmatically use `resolveSkillsetDir` directly so they never warn.
