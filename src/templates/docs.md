...
- Build-time skillset generation from base templates. Takes template variables (agent identity, credential integration ID, etc.) and produces a complete skillset directory ready for review and upload to the registry.
...
- Generated output is intended for upload to the nori-skillsets registry at noriskillsets.dev or for inclusion in the appropriate downstream repo (e.g., nori-sessions for agent-specific session skillsets).
...
- Generated skillsets are typically hand-edited after generation to add agent-specific details (e.g., an agent may require additional flags in its credential handoff). The template provides the skeleton; the developer fills in specifics.
