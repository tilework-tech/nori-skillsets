...
**Registrar API:** The registrar.ts module is a standalone API client for Nori package registries. Unlike other API modules that use apiRequest() with Firebase authentication, the registrar uses direct fetch() calls. It provides four functions: `searchPackages()` searches for profile packages by query string, `getPackument()` retrieves npm-compatible package metadata (packument) including version tags and tarball URLs, `downloadTarball()` downloads a package tarball (resolving to latest version if none specified), and `uploadProfile()` uploads a profile tarball with authentication.

The read functions (`searchPackages`, `getPackument`, `downloadTarball`) accept optional `registryUrl` and `authToken` parameters to support private registries. When `registryUrl` is not provided, functions default to the public registry at `REGISTRAR_URL` (https://registrar.tilework.tech). When `authToken` is provided, it is sent as a Bearer token in the Authorization header. The `uploadProfile` function always requires authentication and only uploads to the public registry. The registrar API is used by the nori-registry-search, nori-registry-download, and nori-registry-upload intercepted slash commands in @/plugin/src/installer/features/hooks/config/intercepted-slashcommands/.
