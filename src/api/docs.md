...
**Transcript API:** The transcript.ts module uploads session transcripts to the user's private registry. It exports `transcriptApi.upload({ sessionId, messages, title? })` which posts to `/transcripts` endpoint using the standard apiRequest pattern with Firebase authentication. The `TranscriptMessage` type is a flexible structure containing optional fields: `type`, `sessionId`, `message` (with role/content), `summary`, and arbitrary additional fields via index signature. The upload response returns `{ id, title, sessionId, createdAt }`. This API is consumed by the watch daemon's uploader module (@/src/cli/commands/watch/uploader.ts) for automatic transcript persistence.
