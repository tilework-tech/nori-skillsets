...
without requiring manual configuration each time.
...
```
nori-broker onboarding handoff-provider --provider {{agentId}} --type skip
```
...
The `{{authIntegrationId}}` integration should show `configured: true` unless the user skipped.
