...
- `@/tests/helpers/` holds shared test utilities that are not vitest files themselves but are loaded into child processes spawned by tests. See below.
...
- **Template generation tests** (`templates/generate.test.ts`): Tests for `@/src/templates/generate.ts`. Verifies that generation produces the expected file tree, substitutes all placeholders in both content and filenames, writes correct manifest fields, rejects missing variables and pre-existing output directories, and produces deterministic output for identical inputs.
- **Module load tracer** (`helpers/moduleLoadTracer.mjs` + `helpers/registerModuleLoadTracer.mjs`): An ESM loader hook that appends every loaded module URL to the file path in `$NORI_LOAD_TRACE`. Tests spawn the built CLI via `node --import ./tests/helpers/registerModuleLoadTracer.mjs ...` (the `register*` shim calls `node:module.register()` to activate the hook), then read the trace file and assert on which modules did or did not appear. `@/src/cli/nori-skillsets.lazy-loading.test.ts` uses this to verify that heavy command handler modules (e.g., `commands/login/login.js`, `commands/registry-*/*.js`) are absent from the import graph for `--version` and present for commands that actually invoke them.
