...
The repo also contains a skillset templating system for scaffolding new agent skillsets. Base templates in `@/templates/` are processed by `@/src/templates/generate.ts` to produce skillset source directories that are uploaded to the registry or committed in the appropriate downstream repo (e.g., nori-sessions). This is a development-time workflow, separate from the runtime CLI.
...
User runs CLI command                   Developer scaffolds new agent
  ...
  @/src/cli/features/*                          |
  @/src/norijson/*                              v
  @/src/providers/*                      generated output dir
  @/src/utils/*                                 |
                                                v
                                         sks upload --> registry
```
