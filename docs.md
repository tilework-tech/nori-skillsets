...
Nori Skillsets is a CLI client and plugin package for installing and managing "skillsets" -- complete, packaged agent configurations for AI coding agents. It supports many agents (Claude Code, Cursor, Codex, and more) through a data-oriented `AgentConfig` type with shared operation functions; each agent is declared as one row in a single declarative table. It connects to the noriskillsets.dev registry to search, download, and upload skillsets and individual skills, while a parallel Git-backed path supports installing, creating, independently forking, and deliberately publishing local skillsets. The package is published to npm as `nori-skillsets` with binary aliases `nori-skillsets`, `nori-skillset`, and `sks`.
...
**Git-backed skillset publication:** `sks publish <skillset> --to <remote>` is the producer-side boundary paired with Git installation. It resolves an existing local profile through the shared namespaced/default-org edge, requires that profile to be its repository root, stages and validates the complete non-ignored workspace, displays the sanitized staged diff, and commits the exact reviewed tree with ordinary Git identity and hooks. The shared tracked-entry validator in `@/src/cli/features/gitPackage.ts` keeps install and publish aligned on the exact root manifest plus rejection of Registry provenance, symbolic links, and submodules; publish additionally requires all declared skill/subagent dependencies to be materialized and rejects slash-command dependencies. It pushes only `HEAD:refs/heads/skillsets/<slug>` with normal fast-forward protection. It does not force, configure remotes, call Registrar, activate content, or persist trust. Before a commit exists, failures restore the original index exactly; after a commit exists, verification or push failures retain it for explicit reconciliation.
